...
from bayesbridge.global_scale_sampler.log_marginal_likelihood_calculator_chol_np import LogMarginalLikelihoodCalculatorCholNP
...
def test_eval(y, obs_prec, X, prior_coef_mean, prior_coef_var, prior_slab_size):
    ...
    calc = LogMarginalLikelihoodCalculatorCholNP(y, design, np.sqrt(prior_intrcpt_var), np.sqrt(prior_coef_var),
...
def test_eval_no_intrcpt(y, obs_prec, X, prior_coef_mean, prior_coef_var, prior_slab_size):
    ...
    regularized_prior_coef_var = prior_slab_size ** 2 * prior_coef_var / (
                prior_slab_size ** 2 + gscale ** 2 * prior_coef_var)
    calc = LogMarginalLikelihoodCalculatorCholNP(y, design, None, np.sqrt(prior_coef_var),
                                               prior_coef_mean, obs_prec, prior_slab_size)
    y_centered = y - gscale * calc.outcome_mean_from_prior
    n, p = X.shape
    ...
    exponent = y_centered @ inv_marg_lik_cov_no_intrcpt @ y_centered
    assert np.isclose(calc(gscale), -0.5 * log_det + -0.5 * exponent)