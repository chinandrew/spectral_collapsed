...
c. Rename unzipped files correctly since some don't have the EUR prefix.
```
import glob
import os
...
for f in glob.glob("*.bim"):  
    os.system("awk '{split($2, id, \":\"); $2 = id[1]; print}' " + f + " > " + f.replace(".bim", "_id.bim"))
```
...
a. Run PRS
Currently counting allele1 (arbitrary choice)
```
...
np.random.seed(2)
df["OBSERVED"] = df.SCORESUM + np.random.normal(0, 40, size=120000)  # about 0.197 signal to noise
# df["OBSERVED"] = (df.SCORESUM + np.random.normal(0, 40, size=120000) >60).astype(int) + 1 # binary version with 9.3% prevalance
print(np.var(df.SCORESUM)/np.var(df.OBSERVED))
...
# 6. p-value LinReg filter 
a. Run regressions
`--allow-no-sex` is so it doesn't treat sex as missing and not run.
```
plink --bfile all_merged --pheno observed_phenotype.txt --logistic --allow-no-sex --out reg_results
```

b. Clumping
```
plink --bfile all_merged --clump-p1 0.001  --clump reg_results.assoc.logistic --out clumped 
...
# for logreg since previously was coded as 1/2 instead of 0/1
y -= 1
