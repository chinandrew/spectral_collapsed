...
def skewed_shrinkage_rejection_sampler(a: float, c: float, q: float, k1: int, k2: int):
    ...
    while not accepted:
        ...
        accept_prob = np.exp((target_log_prob - bound_log_prob).astype(float))
        accepted = accept_prob > np.random.uniform(0, 1)
    return np.squeeze(rv), i
...
def mass_log_convex_c_pos(m1, m2, k, a, c):
    ...
    # print("m_convex")
    # print(m_convex)
    log_den_convex = np.array([log_prob_c_pos(x=m_val, a=a, c=c) for m_val in m_convex])
    # print("log_den_convex")
    # print(log_den_convex)
    den_convex = np.exp(log_den_convex)
    # print("log_prob_c_pos")
    # print(log_prob_c_pos(x=m_convex, a=a, c=c))
    # print(den_convex)
    d_convex = (log_den_convex[1:(k + 1)] - log_den_convex[:k]) / (m_convex[1:(k + 1)] - m_convex[:k])
    # print(den_convex[0])
    # print(den_convex[k])
    if np.abs(den_convex[0] - den_convex[k]) < 2 ** -32:  # almost flat
        coef = np.zeros(k)
        int_convex = den_convex[0] * (m_convex[1:(k + 1)] - m_convex[:k])
        # print("almost flat")
    else:
...
def sample_c_positive(a: float, c: float, q: float, k1: int, k2: int):
    ...
    x = np.log(1 + x_transformed ** 2.0) / 2.0
...
def main():
    ...
    print("c positive")
    c = 1.9879855522847937e+29
    a = 1.856381333291152e-29
    Nsim = 10
    q = 0.5
    k1 = 16
    k2 = 5
    ...
    # print("c negative")
    # a = 7.888609052210118e-156
    # c = -9.749310475075391e+153
    # Nsim = 10
    # q = 0.5
    # k1 = 16
    # k2 = 5
