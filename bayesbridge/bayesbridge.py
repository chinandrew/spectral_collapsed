...
from .global_scale_sampler.coef_full_sampler import coef_full_sampler
...
class BayesBridge():
    ...
    def __init__(self, model, prior=RegressionCoefPrior()):
        ...
        prior.sd_for_unshrunk = prior.sd_for_fixed.copy()
            # Somewhat strange for the constructor to add an instance var to
            # the prior object, but can't think of a better place to store this.
        if model.intercept_added:
            prior.sd_for_unshrunk = np.concatenate((
                [prior.sd_for_intercept], prior.sd_for_unshrunk
            ))
    ...
    def gibbs_resume(
        ...
        self.reg_coef_sampler = SparseRegressionCoefficientSampler(
            self.n_pred, self.prior, coef_sampler_type, curvature_est_stabilized
        )
    ...
    def gibbs(self, n_iter, n_burnin=0, thin=1, seed=None,
              ...
              options=None, _add_iter_mode=False, gscale_method="inv_cdf", proposal_sd=0.5):
        ...
        coef, _ = self.update_regress_coef(
        ...
        for mcmc_iter in range(1, n_iter + 1):
            ...
            coef, obs_prec, gscale, lscale, info \
                = self.gibbs_cycle(coef, obs_prec, gscale, lscale, options, gscale_method, proposal_sd=proposal_sd)
        ...
        mcmc_info = {
            ...
            'prior_sd_for_unshrunk': self.prior.sd_for_unshrunk,
    ...
    def initialize_obs_precision(self, init, coef):
        if 'obs_prec' in init:
            ...
            if self.model.name == "linear":
                obs_prec = init['obs_prec']
            elif self.model.name == 'logit':
                # obs_prec = np.ones(self.n_obs)
                obs_prec = init['obs_prec'].copy(order='C')
                # Cython requires a C-contiguous array.
                valid_valued = (len(obs_prec) == self.n_obs)
            valid_valued = valid_valued and np.all(obs_prec > 0)
            if not valid_valued:
                raise ValueError(
                    'An invalid initial state for observed outcome precision.'
                )
    ...
    def gibbs_cycle(self, coef, obs_prec, gscale, lscale, options, gscale_method="inv_cdf", proposal_sd=0.5):
        ...
        if self.prior.name == "bridge":
            ...
            if gscale_method in ("mh_chol", "mh_eig", "inv_cdf_eig", "inv_cdf_chol"):
    ...
    def update_regress_coef(self, coef, obs_prec, gscale, lscale, sampling_method):
        ...
        if sampling_method in ('cholesky', 'woodbury', 'cg'):
            ...
            coef, info = self.reg_coef_sampler.sample_gaussian_posterior(
                y_gaussian, self.model.design,
                obs_prec, gscale, lscale,
                sampling_method
            )
    ...
    def compute_posterior_logprob(self, coef, gscale, lscale, obs_prec, bridge_exp=None):
        ...
        prior_logp += - 1 / 2 * np.sum(
            (coef[:self.n_unshrunk] / self.prior.sd_for_unshrunk) ** 2
        )
        prior_logp += - np.sum(np.log(
            self.prior.sd_for_unshrunk[self.prior.sd_for_unshrunk < float('inf')]
        ))
        prior_logp += np.log(self.prior.global_scale_prior(gscale))
