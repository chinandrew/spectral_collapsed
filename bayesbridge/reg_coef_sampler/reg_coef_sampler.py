...
class SparseRegressionCoefficientSampler():
    ...
    def sample_gaussian_posterior(
        ...
        """
        Sample from the Gaussian posterior taking advantage of the general formula
        for posterior precision :math:`\Phi_\mathrm{post} = \Phi + \Phi_0` and
        posterior mean :math:`\Phi_\mathrm{post}^{-1} (X^T \Phi y + \Phi_0 \mu_0)`
        under a Gaussian prior with mean :math:`\mu_0` and precision :math:`\Phi_0`
        and likelihood :math:`y \\mid X, \\beta, \\Phi \\sim \\mathcal{N}(X \\beta, \\Phi^{-1})`.
        The function first computes the effective prior mean and precision from
        `lscale`, `gscale`, and other relevant parameters. It then computes the
        :math:`z = X^T \Phi y + \Phi_0 \mu_0` part of the posterior and delegates
        the rest of computation to one of the optimized functions to sample from
        :math:`\mathcal{N}(\Phi_\mathrm{post}^{-1} z, \Phi_\mathrm{post}^{-1})`
        for :math:`\Phi_\mathrm{post} = X^T \Omega X + \Phi_0`.
        ...
        if design.use_cupy:
            if np.isscalar(obs_prec):
                obs_prec = cp.asarray(obs_prec * np.ones(y.size)) # Keeping `obs_prec` a scalar is probably a better design, but putting a temporary fix in place to make the code continues to support cupy w/o changes elsewher
            y = cp.asarray(y)

        prior_mean_for_shrunk = gscale * lscale * self.prior.skew_mean
        prior_sd_for_shrunk = gscale * lscale * self.prior.skew_sd
        prior_mean_for_shrunk, prior_sd_for_shrunk = \
            self.adjust_for_regularization(prior_mean_for_shrunk, prior_sd_for_shrunk)
        prior_sd = np.concatenate((
            self.prior.sd_for_unshrunk, prior_sd_for_shrunk
        ))
        prior_prec_sqrt = 1 / prior_sd

        z = design.Tdot(obs_prec * y)
        z[self.n_unshrunk:] += prior_sd_for_shrunk ** -2 * prior_mean_for_shrunk

        info = {}
        if method == 'cholesky':
            coef = generate_gaussian_with_weight(
                design, obs_prec, prior_prec_sqrt, z
            )
            ...
            obs_prec = obs_prec * np.ones(y.size) # Keeping `obs_prec` a scalar is probably a better design, but putting a temporary fix in place to make the code continues to work w/o changes elsewhere
            coef_condmean_guess = \
    ...
    def sample_by_hmc(
        ...
        precond_scale, precond_prior_prec = \
            self.compute_preconditioning_scale(
                gscale, lscale, coef_precond_post_sd, self.prior.sd_for_unshrunk
            )
    ...
    def compute_preconditioning_scale(
        ...
        prior_sd_for_shrunk = gscale * lscale
        _, precond_scale[n_unshrunk:] \
            = self.adjust_for_regularization(0, prior_sd_for_shrunk)
        if n_unshrunk > 0:
    ...
    def adjust_for_regularization(self, prior_mean, prior_sd):
        regularized_sd = \
            prior_sd / np.sqrt(1 + (prior_sd / self.prior.slab_size) ** 2)
            # Calculation accounts for the case `slab_size == float("inf")`
        regularized_mean = (regularized_sd / prior_sd) ** 2 * prior_mean
        return regularized_mean, regularized_sd
