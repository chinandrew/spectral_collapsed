...
class SparseDesignMatrix(AbstractDesignMatrix):
    ...
    def __init__(self, X, use_mkl=True, center_predictor=False, add_intercept=True,
        ...
        if center_predictor: # account for centering, not centering is equal to set this to be 0s (zeros)
    ...
    def dot(self, v): # account for the centering
        if self.memoized:
    ...
    def compute_unweighted_fisher_info(self, diag_only):
        n_obs = self.X_main.shape[0]
        if diag_only:
            if self.unweighted_fisher_diag is None:
                self.unweighted_fisher_diag \
                    = self.compute_fisher_diag(np.ones(n_obs))
            ...
            if self.unweighted_fisher_info is None:
                ...
                self.unweighted_fisher_info \
                    = self.compute_fisher_info(np.ones(n_obs), diag_only=False)
    ...
    def compute_transposed_fisher_info(self, weight, include_intrcpt=False):
        ...
        weighted_X_T = X.multiply(weight_ex_intrcpt).T.tocsc()
        ...
        if self.centered:
            ...
            transposed_fisher_info -= offset_weight_X[:, np.newaxis]
            transposed_fisher_info \
                += np.sum(weight_ex_intrcpt * self.column_offset ** 2)
        if include_intrcpt:
            transposed_fisher_info += weight[0]
